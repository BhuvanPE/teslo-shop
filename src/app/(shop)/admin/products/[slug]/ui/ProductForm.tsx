...
// Partial<Product> hace que acepte un objeto cuyas propiedas sean  undefined
interface Props {
    product: Partial<Product> & { ProductImage?: ProductImage[] }, // hacemos que el product sea de tipo Product y puede tener un campo ProductImage del tipo indicado
    categories: Category[],
}
...
export const ProductForm = ({ product, categories }: Props) => {
    ...
    const { handleSubmit, register, formState: { isValid }, getValues, setValue, watch } = useForm<FormInputs>({
        defaultValues: {
            ...
            tags: product.tags?.join(', '), // se agrego el Partial en product
            sizes: product.sizes ?? [],
            images: undefined,
        }
    })

    watch('sizes') // para redibucar el componente si cambia la propiedad sizes

    const onSizeChange = (size: string) => {
        // voy a crear un nuevo ser de datos que no acepta duplicados, los elimina
        const sizes = new Set(getValues('sizes'))
        sizes.has(size) ? sizes.delete(size) : sizes.add(size)
        //console.log(Array.from(sizes))
        setValue('sizes', Array.from(sizes)) // convierto el set de datos en un array
    }

    const onSubmit = async (data: FormInputs) => {
        //console.log({ data })
        const formData = new FormData()
        const { images, ...productToSave } = data // destructuo todo el contenido de la data en productToSave
        ...
        const { ok, product: productDB } = await createUpdateProduct(formData)
        //console.log({ ok })
        if (!ok) {
